...
## 17. Phase 19: 宏观大波段与缠论结构趋势架构实证转型 / Phase 19: Macro Structural Trend & Chan Lun Wave Architecture

### 1. 动因与交易本质反思 / Motivation & The Essence of Crypto Trading
用户直指系统最根本痛点：“**第一，对方上涨的时候我们吃不到它上涨；第二，对方下降的时候我们也会出现较大的回撤。感觉甚至不如一些趋势跟踪（布林带）或禅论看图**”。
逐笔复盘证实：
1. **2024 年 3 月 SOL 暴涨 +69.4%**：Transformer 模型在 35 天内交易 14 次，甚至逆势做空 5 次，被扫止损 4 次，最终仅获利 **+1.22%**！而经典趋势跟踪单笔吃下 **+54.1%**！
2. **底层死因**：4h 预测目标充斥白噪声，模型持仓仅 1.2 天便因局部回踩仓皇平仓，且 800 次交易磨掉 62.8% 手续费；连损降仓与回撤降仓产生“亏在 1.0x、反弹在 0.25x”的非对称毒药。

---

### 2. 宏观结构趋势引擎设计 (`crypto_quant/structural_trend_engine.py`)
- **超长持仓大波段**：将持仓周期拉长至 **5~25 天**，只要未触碰结构移动止损，坚决死抱主升浪；
- **自适应移动跟踪止损 (Trailing Stop)**：最高价回撤 $3\times\text{ATR}$ 动态上移保护利润，彻底废除 -2.5% 脆弱硬止损；
- **宏观 200 EMA 趋势定仓**：顺应宏观大周期配置 1.0x / 0.5x 仓位。

---

### 3. 全景实测对比结果 (单一数据源 `docs/trend_vs_transformer_benchmark.json`)
运行标准化实验脚本：
```bash
python scripts/benchmark_trend_vs_transformer.py
```
...
#### 4. 2024–2026 严谨去偏差样本外评测 (单一数据源 `docs/chan_transformer_clean_benchmark.json`)
...
### 5. Phase 21: 方法论严谨修正、去偏差与模型增量价值消融实验 (`docs/model_incremental_value.json`)

系统执行了同口径六组对照消融实验与预测能力反向检验：

#### 1. 6 组对照消融实验结果对比 (50/50 Portfolio)
- **Config 1: 纯结构趋势 (Pure Structure)**: 收益 **+107.36%**, 夏普 **1.27**, 回撤 **-25.26%**, 交易 64 笔, 卡玛 **1.24** (全场最优基石)
- **Config 2: 结构 + 缠论中枢规则 (No ML)**: 收益 **+109.61%**, 夏普 **1.06**, 回撤 **-33.45%**, 交易 130 笔 (收益持平但回撤放大)
- **Config 3: 结构 + 线性逻辑回归门控**: 收益 **+62.43%**, 夏普 **0.80**, 回撤 **-34.60%**, 交易 102 笔 (频繁错失主升浪)
- **Config 4: 结构 + 4h Transformer 门控**: 收益 **+40.02%**, 夏普 **0.61**, 回撤 **-39.95%**, 交易 101 笔 (短周期白噪点误伤趋势)
- **Config 5: 结构 + ST-ChanTransformer**: 收益 **+80.73%**, 夏普 **0.99**, 回撤 **-22.10%**, 交易 81 笔 (回撤收窄，但 SOL 收益明显被压低)
- **Config 6: 结构 + 随机置换门控 (Placebo)**: 收益 **+104.34%**, 夏普 **1.11**, 回撤 **-24.89%**, 交易 111 笔 (随机置换跑赢真实预测)

#### 2. ST-ChanTransformer 预测能力反向检验 (2024–2025 vs. 2026)
- **ETH 12d 预测**: 2024–2025 样本外 Rank IC 为 **+0.1062** (方向命中率 52.2%)，但在 2026 年急剧反转恶化至 **-0.3603** (命中率降至 42.8%)！负向滚动 IC 频率达到 **55.1%**！
- **SOL 12d 预测**: 全周期 Rank IC 为 **-0.0204**，2026 年 Rank IC 为 **-0.1748**，负向滚动 IC 频率达到 **70.8%**！
- **实证定论**：深度模型在未见极端宏观周期发生严重泛化失效，未能证明稳定增量 Alpha。纯结构趋势策略具备更强的抗脆弱性。

#### 3. Phase 19 纯结构趋势深度严谨实证总结 (`docs/structural_trend_clean_benchmark.json`)
- **盈利集中度检验 (Leave-one-out)**：
  - 剔除最大 1 笔盈利交易后，ETH 累计复合收益仍达 **+64.3%**，SOL 仍达 **+34.2%**，策略依然盈利，非单一交易偶然支撑。
- **超大参数邻域平滑度**：
  - ETH 100.0% 参数组合盈利（收益 [+55.0%, +147.1%]，中位数夏普 1.14）；SOL 85.7% 参数组合盈利；不存在孤立峰值。
- **真实步进滚动 Walk-Forward 样本外拼接曲线**：
  - 历史 24 个月选参 $\to$ 冻结参数推演随后 3 个月测试窗 $\to$ 滚动拼接：ETH 真实 OOS 夏普达 **0.83**（远高于 $\ge 0.50$ 门槛），总回报 **+64.27%**！
- **手续费极限压力测试 (5 bps 至 30 bps)**：
  - 在单边 15 bps 下组合收益 +98.36% (夏普 1.20)；在极端惩罚性的 **单边 30 bps (双边 60 bps)** 摩擦下，组合收益依然达到 **+80.35%，夏普达 1.04**！

*最终定级与建议*：
- **Phase 19 纯结构趋势跟踪（Pure Structural Trend）**：通过全部 8 项审核标准，**获批作为首要候选策略进入冻结参数模拟实盘（Forward Paper Testing）**；
- **Phase 20 ST-ChanTransformer**：明确定位为**实验性科研原型（Experimental Filter）**，不作为实盘主推方案。
