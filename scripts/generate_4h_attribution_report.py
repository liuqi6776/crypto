...
def generate_report():
    ...
    sizing_csv = report_dir / "isolated_sizing_ablation.csv"
    ...
    df_sizing = pd.read_csv(sizing_csv) if sizing_csv.exists() else pd.DataFrame()
    ...
    with open(out_md, "w", encoding="utf-8") as f:
        ...
        f.write("> 4. **Controlled Single-Variable Ablations / 严格受控单变量消融**: Evaluated pure entry channels under 100% identical exit rules, and evaluated macro sizing independently holding channel constant.\n")
        ...
        f.write("---\n\n")
        f.write("## 3. Controlled Single-Variable Ablations / 严格受控单变量消融实验\n\n")
        f.write("### 3.1 Pure Entry Channel Ablation with Fixed Exit Rules (Bollinger 120 vs Donchian 120)\n")
        f.write("### 3.1 固定退场规则的纯入场通道单变量消融（布林带 120 vs 唐奇安 120 严格对照）\n\n")

        if not df_ablation.empty:
            f.write(df_ablation.to_markdown(index=False))
            f.write("\n\n")
            f.write("> **Methodological Clarifications & Empirical Observations / 方法论澄清与实证观察**:\n")
            f.write("> 1. **Accurate Definition of Donchian Upper / 唐奇安上轨准确定义**: In code, Donchian upper is calculated as `c.shift(1).rolling(120).max()`, which represents the 120-period rolling **highest Close (最高收盘价)**, NOT highest High. Lower band is 120-period lowest Low.\n")
            f.write("> 2. **Fixed-Exit Pure Entry Controls / 退场规则完全固定的纯入场对照**:\n")
            f.write(">    - **Regime 1 (Shared BB Mid & 3.0 ATR Stop)**: Both variants exit strictly when `Close < bb_mid` (120-period MA) or trailing stop. Bollinger 120 achieves **+2,220.31%** (Sharpe 1.40, 158 trades) vs Donchian 120 at **+1,560.21%** (Sharpe 1.30, 183 trades). Observed Difference: **+660.10 percentage points (个百分点)**.\n")
            f.write(">    - **Regime 2 (Shared 3.0 ATR Stop Only, No Mid-Band Exit)**: Both variants exit strictly on trailing stop. Bollinger 120 achieves **+2,559.86%** (Sharpe 1.44, 157 trades) vs Donchian 120 at **+1,556.41%** (Sharpe 1.30, 183 trades). Observed Difference: **+1,003.45 percentage points (个百分点)**.\n")
            f.write(">    - **Regime 3 (Mode B Intrabar 3.0 ATR Stop Touch)**: Exit occurs solely when intrabar Low touches the prior-bar stop. Bollinger 120 achieves **+860.05%** (Sharpe 1.12, 205 trades) vs Donchian 120 at **+190.89%** (Sharpe 0.68, 229 trades). Observed Difference: **+669.16 percentage points (个百分点)**.\n")
            f.write("> 3. **Trade Frequency, Friction Amount, and Return Divergence / 交易频次、摩擦金额与收益分歧定性**:\n")
            f.write(">    - 在三个固定退场对照中，唐奇安通道触发了更多交易笔数（多 24 至 26 笔），但其全周期**累计摩擦金额反而更低**（例如共享布林中轨下为 $15,750，低于布林带的 $20,333；无中轨止损退出下为 $15,709，低于布林带的 $23,178；Mode B 下为 $7,545，低于布林带的 $17,889）。实证表明：**在变动权益回测中，交易笔数更多绝不必然导致更高的累计摩擦金额**。“策略净值复合增长较低导致后续单笔名义开仓金额与摩擦变小”是一个合理解释/假说，但仅凭总摩擦汇总表尚不能确认主导原因，仍须结合逐笔成交金额与换手数据做进一步实证核实。\n")
            f.write(">    - 唐奇安版本在本次回测中表现较弱的直接表现是胜率较低（38.4%~42.6% 对比 42.4%~46.5%）和盈亏比更低（1.31~1.89 对比 1.64~2.06）。导致这两组入场触发时点盈亏分歧的深层机理，不能简单推断为摩擦损耗，而需要未来进一步开展逐笔交易的微观收益拆解。此外，上述收益差（如 660.10 个百分点）均为本次特定历史样本的条件对照结果，不能当作未来交易的确定性超额。\n\n")

        f.write("### 3.2 Standalone EMA200 Macro Sizing Ablation (Fixed Bollinger 120 Channel)\n")
        f.write("### 3.2 独立 EMA200 宏观定仓单变量消融（固定布林带 120 通道）\n\n")

        if not df_sizing.empty:
            f.write(df_sizing.to_markdown(index=False))
            f.write("\n\n")
            f.write("> **Empirical Sizing Contribution & Mechanistic Clarification / 宏观定仓规则独立贡献与机制准确定性**:\n")
            f.write("> - Holding the Bollinger 120 channel and stop rules strictly identical:\n")
            f.write(">   - **Mode A (Bar-Close Exit)**: Fixed Full Sizing (1.0 constant) yields **+2,113.72%** (Sharpe 1.39, Max DD 40.39%). EMA200 Half Sizing (1.0 / 0.5) achieves **+2,220.31%** (Sharpe 1.40, Max DD 39.71%). Observed Difference: **+106.59 percentage points (个百分点)**, with drawdown reduced by 0.68 percentage points (个百分点).\n")
            f.write(">   - **Mode B (Intrabar Touch)**: Fixed Full Sizing yields **+791.83%** (Sharpe 1.09, Max DD 33.71%). EMA200 Half Sizing achieves **+860.05%** (Sharpe 1.12, Max DD 32.44%). Observed Difference: **+68.22 percentage points (个百分点)**, with drawdown reduced by 1.27 percentage points (个百分点).\n")
            f.write(">   - **Binary Macro Gate (1.0 / 0.0)**: Yields **+2,217.94%** (Max DD 37.48%) in Mode A and **+814.03%** (Max DD 31.05%) in Mode B, demonstrating that blocking entry below EMA200 improves drawdown mitigation at a slight cost to compounding in early recoveries.\n")
            f.write("> - **Mechanistic Precision / 机制实现准确定性**: In code, the EMA200 `macro_mult` (1.0 / 0.5) applies **strictly at new trade entry** to determine target order notional. Once opened, if price drops below EMA200 during the trade, the existing position is **NOT** automatically reduced retroactively (it is held until trailing stop or mid-line exit). Therefore, this rule operates strictly by **reducing new position capital allocation when entering below EMA200 (低于 EMA200 时减少新仓投入)**, rather than dynamically scaling down portfolio exposure mid-trade.\n")
            f.write("> - **Conclusion / 归因裁决**: In this historical sample, reducing new position sizing when below EMA200 provided a modest positive difference (+68.22 to +106.59 percentage points; 0.68 to 1.27 percentage points drawdown improvement). This is a conditional backtest observation on the historical dataset and cannot be treated as a forecast of future performance, nor does it account for the entire divergence across broader strategy iterations.\n\n")
        ...
        f.write("- Incremental annual friction incurred in 2026 is **$471,467.22**, and total full-cycle friction is **$1,742,428.48**. The strategy generates sufficient cross-sectional alpha to more than overcome turnover friction. However, capacity constraints are estimated to become binding above ~$250k portfolio size; this threshold must be treated as an unverified rough estimate derived from static orderbook snapshots, rather than a proven hard ceiling during live trade execution.\n")
        f.write("- 单账本纠偏后，Top-1 轮动策略 2026 年实际发生的增量摩擦成本为 47.1 万美元（全周期累计为 174.2 万美元），平均现金仓位为 48.5%。其截面动量超额在扣费后依然超越被动基准与简单趋势；但在大资金体量下可能受到盘口容量耗尽的制约——需要说明的是，“资金规模超过约 25 万美元时容量受限”仅为基于单次静态盘口快照的**初步待验证估计**，因快照无法代表交易瞬间的动态成交容量，确切资金容量仍需依赖前向持续深度实测核实。\n\n")
